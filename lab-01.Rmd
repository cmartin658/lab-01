...
```
# views first 10 rows in data set
visitors %>% head(n = 10)
...
## Question 5
```{r Q5}
filter(visitors, setting == 'O', region == 'Yorkshire and the Humber', admission == 'Members', n_2022 > 100000) %>%
  count()
```
...
**not applicable, no data for that part. since it does not know some parts of the data it cannot find any averages, so it also sets them to NA. in other words, any operation with NA outputs NA**
...
**this could be because there is a large range of values that can warp the mean, but has little effect on the median**
